...
def sdt_from_df(df):
    ...
    return HR, FAR, d, c, p_correct, catRT
...
def get_sdt_msr(n_rec_signal, n_signal, n_rec_noise, n_noise):
    ...
    if (n_noise == 0):
        ...
        FAR = max(float(n_rec_noise) / n_noise, 1. / (2 * n_noise))
    if n_signal == 0:
        HR = np.nan
    else:
        HR = min(float(n_rec_signal) / n_signal, 1 - (1. / (2 * n_signal)))
    d = Z(HR) - Z(FAR)
    c = -(Z(HR) + Z(FAR)) / 2.
...
def fill_dict_w_bhv(HR, FAR, d, c, p_correct, catRT, subject, group, IV):
    bhv_group_val = {}
    bhv_group_val['HR'] = [HR]
...
def get_SDT_by_IV(bhv_df, IV):
    ...
    for subject in subjects:
        ...
        for group in np.unique(p_group):
            ...
            HR, FAR, d, c, p_correct, catRT = sdt_from_df(group_df)
...
def test_blink_effects_on_SDT():
    ...
    for subject in HLTP_pupil.subjects:
        ...
        for blink in [True, False]:
            HR, FAR, d, c, p_correct, catRT = sdt_from_df(
...
# Run the functions
df_name = 'all_subj_bhv_df_w_pupil_power'# prepare this df with 4_DICS_roi_analysis
#df_name = 'all_subj_bhv_df_w_pupil'
bhv_df = pd.read_pickle(HLTP_pupil.result_dir +
                          '/' + df_name + '.pkl')
